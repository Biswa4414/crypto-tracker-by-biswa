  ...
  return (
    <a href={`/coin/${coin.id}`}>
      ...
      >
        ...
        {/* Tooltips are a common user interface element that appears when the user hovers over 
        or interacts with another element, typically providing explanatory text or supplementary details. */}
        ...
        <Tooltip placement="bottom-start" title="Price">
          <td className="td-price-chip-list">
            {coin.price_change_percentage_24h > 0 ? (
              ...
              </div>                                                                  //.toFixed() method is used to format the price change percentage
            //                                                                         as a string with exactly two decimal places & add % symbol
            ) : (
              <div className="info-flex" style={{ marginBottom: 0 }}>
            ...
            >
              ...
              {convertNumbers(
                coin.current_price < 1
                  ? parseFloat(coin.current_price).toFixed(3) //0.859 or 0.999
